...
def find_external_in_single_file(text):
    ...
    for ref in regex.finditer(reference_regex, text, regex.MULTILINE):
        ref_data = {}
        if ref.group('journals') is not None:
            ref_data['journals'] = []
            for year_group in regex.finditer(journal_year_groups_regex, ref.group('journals'), regex.MULTILINE):
                ...
                for number_group in regex.finditer(journal_numbers_regex, year_group.group('numbers'), regex.MULTILINE):
                    ...
                    for position_group in regex.finditer(journal_position_regex, number_group.group('positions'), regex.MULTILINE):
                        position = position_group.group('position')
                        ref_data['journals'].append({'year': year,
                                                     'number': number,
                                                     'position': position})
        else:
            ...
            for year_group in regex.finditer(journal_year_groups_regex, ref.group('journal_details'), regex.MULTILINE):
                ...
                for journal_data in regex.finditer(journal_numbers_regex, year_group.group('numbers'), regex.MULTILINE):
                    for position_group in regex.finditer(journal_position_regex, journal_data.group('positions'), regex.MULTILINE):
